...
export default async function EnquiryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AdminShell unread={unread || 0}>
    ...
    <header className="admin-header detail-header"><div className="lead-title"><span className="lead-avatar large">{enquiry.name.slice(0,2).toUpperCase()}</span><div><p className="admin-kicker">PROJECT ENQUIRY · {enquiry.priority || "Normal"} priority</p><h1>{enquiry.name}</h1><p>Received {new Date(enquiry.created_at).toLocaleString("en-GB")}</p></div></div><div className="header-actions"><span className={`status status-${String(enquiry.status).toLowerCase()}`}>{enquiry.status}</span><a className="admin-button" href={`mailto:${enquiry.email}?subject=${encodeURIComponent("Your Navill Tech project enquiry")}`}><Send/>Reply by email</a>{whatsapp&&<a className="admin-button" href={whatsapp} target="_blank" rel="noopener noreferrer"><MessageCircle/>WhatsApp</a>}</div></header>
    ...
    </section><aside className="admin-card notification-card"><div className="card-heading"><div><p className="admin-kicker">DELIVERY LOG</p><h2>Notifications</h2></div><Bell/></div>{deliveries?.map(delivery=><div className="delivery" key={delivery.id}><span><i className={deliveryOk.has(delivery.status)?"ok-dot":"failed-dot"}/>{delivery.channel}</span><span className={deliveryOk.has(delivery.status)?"ok":"failed"}>{delivery.status}</span></div>)}{!deliveries?.length&&<p className="empty-copy">No notification attempts recorded.</p>}</aside></div>
  </AdminShell>;
}
