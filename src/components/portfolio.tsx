    ...
    <section id="work" className="section work">
      <div className="container">
        ...
        <div className="projects">{projects.map((p,i)=><Reveal key={p.name}><article className={`project project-${p.color}`}>
          <div className="project-copy"><span className="project-index">{p.number} / CASE STUDY</span><div className="project-brand"><span>{p.name[0]}</span><div><b>{p.name}</b><small>{p.type}</small></div></div><h3>{p.title}</h3><p>{p.copy}</p><div className="tags">{p.tags.map(t=><span key={t}>{t}</span>)}</div><div className="result"><Rocket size={18}/><div><small>BUSINESS RESULT</small><b>{p.result}</b></div></div><button className="text-link" data-track={p.name} data-event="project_view">View case study <ArrowRight size={17}/></button></div>
