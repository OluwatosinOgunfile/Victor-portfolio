...
async function recentEvents(days = 30) {
  ...
  const { data, error } = await db.from("analytics_events").select(eventFields).gte("created_at", since).order("created_at", { ascending: true }).limit(50000);
...
export async function getAnalyticsData(days = 30) {
  const db = createServiceClient();
  const [events, unread] = await Promise.all([recentEvents(days), db.from("enquiries").select("id", { count: "exact", head: true }).eq("is_read", false)]);
...
export async function getAdminData(days = 30) {
  const db = createServiceClient();
  const [events, enquiries] = await Promise.all([recentEvents(days), db.from("enquiries").select("is_read")]);
